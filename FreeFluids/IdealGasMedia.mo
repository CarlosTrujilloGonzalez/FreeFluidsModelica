...
package IdealGasMedia "IdealGasMedia.mo by Carlos Trujillo
  ...
  partial package IdealGasMedium
    ...
    redeclare model extends BaseProperties(T(stateSelect = if preferredMediumStates then StateSelect.prefer else StateSelect.default), p(stateSelect = if preferredMediumStates then StateSelect.prefer else StateSelect.default))
      equation
    ...
    redeclare function extends setState_pTX "Return ThermodynamicState record as function of p,T and composition X or Xi"
      ...
      algorithm
        ...
        state.h := SpecificEnthalpyCorr(data.Cp0Corr, data.Cp0Coef, data.MW, T);
    ...
    redeclare function extends specificEntropy "Return specific entropy"
      ...
      algorithm
        s := SpecificEntropyCorr(data.Cp0Corr, data.Cp0Coef, data.MW, state.T) - R * Modelica.Math.log(state.p / reference_p) * 1000 / data.MW;
    end specificEntropy;
    ...
    redeclare function extends specificHeatCapacityCp "Return specific heat capacity at constant pressure"
      ...
      algorithm
        cp := PhysPropCorr(data.Cp0Corr, data.Cp0Coef, data.MW, state.T);
    ...
    redeclare function extends specificHeatCapacityCv "Compute specific heat capacity at constant volume from temperature and gas data"
      ...
      algorithm
        cv := PhysPropCorr(data.Cp0Corr, data.Cp0Coef, data.MW, state.T) - R * 1000 / data.MW;
      annotation(
        Inline = true,
        smoothOrder = 2);
    ...
    redeclare function extends isentropicEnthalpy "Return an approximation of isentropic enthalpy (gamma is considered constant)"
      ...
      function isentropicEnthalpyApproximation=isentropicEnthalpy;
    ...
    redeclare function extends dynamicViscosity "Return dynamic viscosity"
      ...
      algorithm
        ...
        eta := if data.gViscCorr > 0 and useTransportCorr == true then PhysPropCorr(data.gViscCorr, data.gViscCoef, data.MW, state.T) else FreeFluids.MediaCommon.Functions.gasViscLowPressureChung(data, state.T);
    ...
    redeclare function extends thermalConductivity "Return thermal conductivity"
      ...
      algorithm
        ...
        lambda := if data.gThCondCorr > 0 and useTransportCorr == true then PhysPropCorr(data.gThCondCorr, data.gThCondCoef, data.MW, state.T) else FreeFluids.MediaCommon.Functions.gasThCondLowPressureChung(data, specificHeatCapacityCp(state), dynamicViscosity(state), state.T);
  ...
  package Tests
    ...
    model IdealGasH2OA "IdealGas H20 medium model. Adapted from Modelica.Media.Examples.IdealGasH2O"
      ...
      Medium.SpecificEnthalpy h_is = Medium.isentropicEnthalpyApproximation(2.0e5, state);
    ...
    model IdealGasH2OB "IdealGas H20 medium model"
      extends IdealGasH2OA(redeclare package Medium = Modelica.Media.IdealGases.SingleGases.H2O);
    end IdealGasH2OB;
  end Tests;
  annotation(
    Documentation(info = "<html>
