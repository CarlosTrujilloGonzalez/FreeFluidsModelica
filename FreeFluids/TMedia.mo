  ...
  partial package TMedium
    ...
    redeclare function extends saturationTemperature "Return saturation temperature from P"
      ...
      algorithm
        ...
        T := if p >= fluidConstants[1].criticalPressure then fluidConstants[1].criticalPressure else if fluidConstants[1].BtCorr > 0 then PhysPropCorr(fluidConstants[1].BtCorr, fluidConstants[1].BtCoef, fluidConstants[1].MW, p) else saturationPressureInv(p);
    ...
    redeclare function extends setState_pTX "Return ThermodynamicState record as function of p,T and composition X or Xi"
      ...
      algorithm
        ...
        elseif p < Vp then
          ...
          state.h := state.h + SpecificEnthalpyCorr(fluidConstants[1].Cp0Corr, fluidConstants[1].Cp0Coef, fluidConstants[1].MW, T) - SpecificEnthalpyCorr(fluidConstants[1].Cp0Corr, fluidConstants[1].Cp0Coef, fluidConstants[1].MW, Tb) "Gas entahlpy at T and p. Adjustement at T using Cp0";
          if highPressure == true then
            state.h := state.h -( Vp / PhysPropCorr(fluidConstants[1].gSatDensCorr, fluidConstants[1].gSatDensCoef, fluidConstants[1].MW, T) - p / state.d)*4*p/fluidConstants[1].criticalPressure;
          end if;
        else
      ...
      algorithm
        ...
        if d >= Dls then
          ...
          if d >= Dgs then
            ...
            state.gf := 1;
            state.p := d * 1000 * R * T / fluidConstants[1].MW;
            ...
            if state.p>0.1*Vp then
              Dsim := d + (Dgs - d) / (0.9*Vp) * (state.p-0.1*Vp) "ideal gas density at 0 pressure, saturated at Vp";
            else
              Dsim:=d;
            end if;
            //Dsim := d + (Dgs - d) * state.p / Vp;
            state.p := state.p * d / Dsim;
            Dig := state.p * fluidConstants[1].MW / (1000 * R * T);
            Tb := saturationTemperature(state.p);
            Dgs := PhysPropCorr(fluidConstants[1].gSatDensCorr, fluidConstants[1].gSatDensCoef, fluidConstants[1].MW, Tb);
            if state.p>0.1*Vp then
              Dsim := Dig + (Dgs - Dig) / (0.9*Vp) * (state.p-0.1*Vp) "ideal gas density at 0 pressure, saturated at Vp";
            else
              Dsim:=Dig;
            end if;
            //Dsim := Dig + (Dgs - Dig) * state.p / Vp;
            state.p := state.p * d / Dsim;
            Dig := state.p * fluidConstants[1].MW / (1000 * R * T);
            Tb := saturationTemperature(state.p);
            Dgs := PhysPropCorr(fluidConstants[1].gSatDensCorr, fluidConstants[1].gSatDensCoef, fluidConstants[1].MW, Tb);
            ...
            state.h := state.h + SpecificEnthalpyCorr(fluidConstants[1].Cp0Corr, fluidConstants[1].Cp0Coef, fluidConstants[1].MW, T) - SpecificEnthalpyCorr(fluidConstants[1].Cp0Corr, fluidConstants[1].Cp0Coef, fluidConstants[1].MW, Tb) "Gas entahlpy at T and p. Adjustement at T using Cp0";
            if highPressure == true then
              state.h := state.h - (Vp / PhysPropCorr(fluidConstants[1].gSatDensCorr, fluidConstants[1].gSatDensCoef, fluidConstants[1].MW, T) - state.p / state.d)*4*state.p/fluidConstants[1].criticalPressure;
            end if;
          end if;
        end if;
        Href := if refState == "ASHRAE" then SpecificEnthalpyCorr(fluidConstants[1].lCpCorr, fluidConstants[1].lCpCoef, fluidConstants[1].MW, 233.15) else if refState == "IIR" then SpecificEnthalpyCorr(fluidConstants[1].lCpCorr, fluidConstants[1].lCpCoef, fluidConstants[1].MW, 273.15) - 2.0e5 else if refState == "NBP" then SpecificEnthalpyCorr(fluidConstants[1].lCpCorr, fluidConstants[1].lCpCoef, fluidConstants[1].MW, fluidConstants[1].Tb) else if refState == "User" then SpecificEnthalpyCorr(fluidConstants[1].lCpCorr, fluidConstants[1].lCpCoef, fluidConstants[1].MW, reference_T) else 0;
    ...
    redeclare function extends setState_phX "Return ThermodynamicState record as function of p,H and composition X or Xi"
      ...
      protected
        ...
        SpecificEnthalpy HrawCor "Input enthalpy without reference correction, corrected for pressure";
        AbsolutePressure Vp;
        SpecificEnthalpy Href;
        SpecificEnthalpy H0critical;
      ...
      algorithm
        ...
        if Hls >= Hraw then
          state.phase := 1 "liquid";
          state.gf := 0;
          state.T := if fluidConstants[1].lTfromHsatCorr > 0 then PhysPropCorr(fluidConstants[1].lTfromHsatCorr, fluidConstants[1].lTfromHsatCoef, fluidConstants[1].MW, Hraw) else SpecificEnthalpyCorrInv(fluidConstants[1].lCpCorr, fluidConstants[1].lCpCoef, fluidConstants[1].MW, fluidConstants[1].Tc - 0.01, Hraw);
          ...
          if Hraw <= Hgs then
            ...
            state.phase := 1 "gas";
            state.gf := 1.0;
            H0critical := SpecificEnthalpyCorr(fluidConstants[1].Cp0Corr, fluidConstants[1].Cp0Coef, fluidConstants[1].MW, fluidConstants[1].Tc);
            H0b := SpecificEnthalpyCorr(fluidConstants[1].Cp0Corr, fluidConstants[1].Cp0Coef, fluidConstants[1].MW, Tb);
            H0slope := (fluidConstants[1].Tc - Tb) / (H0critical - H0b);
            state.T := Tb + H0slope * (Hraw - Hgs);
            HrawCor := Hraw;
            if highPressure == true then
              for i in 1:6 loop
                Dgi := p * fluidConstants[1].MW / (1000 * R * state.T);
                Vp := PhysPropCorr(fluidConstants[1].VpCorr, fluidConstants[1].VpCoef, fluidConstants[1].MW, state.T);
                state.d := Dgi + (Dgs - Dgi) / (0.9*Vp) * (p-0.1*Vp);
                HrawCor := Hraw + (Vp / PhysPropCorr(fluidConstants[1].gSatDensCorr, fluidConstants[1].gSatDensCoef, fluidConstants[1].MW, state.T) - p / state.d)*4*p/fluidConstants[1].criticalPressure;
                state.T := state.T + 0.7 * H0slope * (HrawCor - Hgs - SpecificEnthalpyCorr(fluidConstants[1].Cp0Corr, fluidConstants[1].Cp0Coef, fluidConstants[1].MW, state.T) + H0b);
              end for;
            else
              H0slope := (state.T - Tb) / (SpecificEnthalpyCorr(fluidConstants[1].Cp0Corr, fluidConstants[1].Cp0Coef, fluidConstants[1].MW, state.T) - H0b);
              state.T:= Tb + H0slope * (Hraw - Hgs);
            end if;
            Dgi := p * fluidConstants[1].MW / (1000 * R * state.T);
            Vp := PhysPropCorr(fluidConstants[1].VpCorr, fluidConstants[1].VpCoef, fluidConstants[1].MW, state.T);
    ...
    redeclare function extends setState_psX "Return thermodynamic state as function of p, s and composition X or Xi"
      ...
      algorithm
        ...
        if Sl >= Sraw then
          state.phase := 1 "liquid";
          state.gf := 0;
          state.T := SpecificEntropyCorrInv(fluidConstants[1].lCpCorr, fluidConstants[1].lCpCoef, fluidConstants[1].MW, fluidConstants[1].Tc - 0.01, Sraw);
          Dls := PhysPropCorr(fluidConstants[1].lDensCorr, fluidConstants[1].lDensCoef, fluidConstants[1].MW, state.T) "saturated liquid density";
          Vp := PhysPropCorr(fluidConstants[1].VpCorr, fluidConstants[1].VpCoef, fluidConstants[1].MW, state.T);
          state.d := if fluidConstants[1].lnuA > 0 then Dls + log(fluidConstants[1].lnuA * fluidConstants[1].MW / (1000 * exp(fluidConstants[1].lnuA * Dls + fluidConstants[1].lnuB) * Modelica.Constants.R * state.T) * (state.p - Vp) + 1) / fluidConstants[1].lnuA else Dls / (1 - fluidConstants[1].IsothComp * (state.p - Vp)) "saturated liquid density, pressure corrected";
          if highPressure == true then
          ...
          if Sg >= Sraw then
            ...
            state.T := SpecificEntropyCorrInv(fluidConstants[1].Cp0Corr, fluidConstants[1].Cp0Coef, fluidConstants[1].MW, 1500.0, Sig + Sexc);
            Dgs := PhysPropCorr(fluidConstants[1].gSatDensCorr, fluidConstants[1].gSatDensCoef, fluidConstants[1].MW, Tb);
            Dgi := p * fluidConstants[1].MW / (1000 * R * state.T);
            Vp := PhysPropCorr(fluidConstants[1].VpCorr, fluidConstants[1].VpCoef, fluidConstants[1].MW, state.T);
            if p>0.1*Vp then
            ...
            state.h := SpecificEnthalpyCorr(fluidConstants[1].lCpCorr, fluidConstants[1].lCpCoef, fluidConstants[1].MW, Tb) "unreference saturated liquid enthalpy at p";
            state.h := state.h + PhysPropCorr(fluidConstants[1].HvCorr, fluidConstants[1].HvCoef, fluidConstants[1].MW, Tb) "saturated gas enthalpy at p";
            state.h := state.h + SpecificEnthalpyCorr(fluidConstants[1].Cp0Corr, fluidConstants[1].Cp0Coef, fluidConstants[1].MW, state.T) - SpecificEnthalpyCorr(fluidConstants[1].Cp0Corr, fluidConstants[1].Cp0Coef, fluidConstants[1].MW, Tb) "Gas entahlpy at T and p. Adjustement at T using Cp0";
            if highPressure == true then
              state.h := state.h - (Vp / PhysPropCorr(fluidConstants[1].gSatDensCorr, fluidConstants[1].gSatDensCoef, fluidConstants[1].MW, state.T) - p / state.d)*4*p/fluidConstants[1].criticalPressure;
            end if;
          end if;
        end if;
        state.h := state.h - Href "referenced enthalpy";
    ...
    redeclare function extends specificEntropy "Return specific entropy"
      ...
      algorithm
        ...
        elseif state.gf < 1.0 then
          ...
          Tb := saturationTemperature(state.p);
          s := SpecificEntropyCorr(fluidConstants[1].lCpCorr, fluidConstants[1].lCpCoef, fluidConstants[1].MW, Tb) "unreferenced saturated liquid enthalpy at p(Tb)";
          s := s + PhysPropCorr(fluidConstants[1].HvCorr, fluidConstants[1].HvCoef, fluidConstants[1].MW, Tb) / Tb "saturated gas enthalpy at p(Tb)";
          s := s + SpecificEntropyCorr(fluidConstants[1].Cp0Corr, fluidConstants[1].Cp0Coef, fluidConstants[1].MW, state.T) - SpecificEntropyCorr(fluidConstants[1].Cp0Corr, fluidConstants[1].Cp0Coef, fluidConstants[1].MW, Tb) "gas enthalpy at T";
        end if;
        Sref := if refState == "ASHRAE" then SpecificEntropyCorr(fluidConstants[1].lCpCorr, fluidConstants[1].lCpCoef, fluidConstants[1].MW, 233.15) else if refState == "IIR" then SpecificEntropyCorr(fluidConstants[1].lCpCorr, fluidConstants[1].lCpCoef, fluidConstants[1].MW, 273.15) - 2.0e5 else if refState == "NBP" then SpecificEntropyCorr(fluidConstants[1].lCpCorr, fluidConstants[1].lCpCoef, fluidConstants[1].MW, fluidConstants[1].Tb) else if refState == "User" then SpecificEntropyCorr(fluidConstants[1].lCpCorr, fluidConstants[1].lCpCoef, fluidConstants[1].MW, reference_T) else 0;
...
//dhvdp:= (-PhysPropCorr(fluidConstants[1].HvCorr, fluidConstants[1].HvCoef, fluidConstants[1].MW, sat.Tsat) + PhysPropCorr(fluidConstants[1].HvCorr, fluidConstants[1].HvCoef, fluidConstants[1].MW, saturationTemperature(0.9*sat.psat)))/(0.1*sat.psat)"needs a high pressure range. As correlations are T functions better not to use.";
  ...
  package Fluids
    ...
    package Hexane_n
      extends FreeFluids.TMedia.TMedium(final mediumName = "n-Hexane", final singleState = false, fluidConstants = {FreeFluids.MediaCommon.MediaDataAL.Hexane_n}, refState="IIR", reference_T=273.15);
    end Hexane_n;
  ...
  package Tests
    ...
    //Construction of StateP and calculation of properties
      for i in 1:1 loop
        ...
        Gamma := Cp/Cv;//Medium.isentropicExponent(StateP);
    ...
    model TestA2A
      extends FluidTestingA(redeclare replaceable package Medium = TMedia.Fluids.R134A(highPressure = true, refState = "ASHRAE", reference_T = 100.0), p = 10.0e5, initialT = (-50.0) + 273.15, finalT = 85.0 + 273.15, fract = 0.5);
    end TestA2A;
...
//Construction of BaseProperties
    ...
    model TestB1A
      extends FluidTestingB(redeclare replaceable package Medium = FreeFluids.TMedia.Fluids.Water(refState = "User", reference_T = 273.15, highPressure = true), T = 100 + 273.15, initialP = 100.0e5, fract = 1.0, finalP = 1.1e5);
    end TestB1A;
  ...
  annotation(
    ...
    <p>The use of pressure correction is controlled by the constant Boolean 'highPressure'. Its default value is false. If switched to true, pressure correction will be applied (in plus than to liquid density) to liquid specific enthalpy, specific entropy, heat capcity, viscosity and thermal conductivity, and for gas enthalpy. It is interesting to make highPressure=true if we need to work over 20 or 30 bars, but the price is a slower simulation.</p>
    ...
    <p>The values of enthalpy and entropy are calculated from a reference state. The reference state to use can be selected giving value to the constant string 'refState'. The values can be:  'ASHRAE', 'IIR', 'NBP' or 'User'. Any other value will eliminate any correction for the reference state. When using 'User', the values at reference_T will be used as zero for both enthalpy and entropy.</p>
    ...
    <p>In the package Tests there is a comparison between the medium performance with water and the Modelica WaterIF97_ph medium model (TestA1A/B and TestB1A/B). And with R134A and the Modelica R134a_ph model. The Modelica R134a_ph model seems incorrect. There is also an example taken from ThermoPower.</p>
